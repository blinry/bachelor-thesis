...
Nowadays, when humans interact with computers, most of the time, they use a graphical user interface. It provides buttons and other visual elements the user can interact with using a mouse. (ref: The Demo?) Before this interaction method was invented, computers had a text-only interface. The user would type a text command, and the machine would execute it. These \emph{Command Line Interfaces} provide powerful, efficient means to interact with computers (finde ich ne ref zu efficient?), 

As CLIs have no means of telling the user what can be done with it, they are not self-evident. This is why they often have a steep learning curve, and a novice user needs to be taught how to use it.

Most approaches to teach command line interfaces contain text: There are books and manuals, online tutorials and exercise sheets. But these text-based mediums pose several problems: The users have to shift their attention back and forth between the explaining text and the system they are learning, which slows down the learning process. The text might set tasks and goals, but has no possibility to check and confirm when the user reaches them. Finally, when the user makes a syntactical or semantical mistake, he is on his own---the text remains static and cannot help him.

This thesis describes a system that provides a much more direct, interactive teaching approach. The core idea is to interlace the tutorial text and the command line output and to make the tutorial watch the user's commands, the commands' output and the state of the commnd line system, so that the text can respond directly to the user's actions.

This approach is inspired by text adventures. \Cref{fig:zork} demonstrates one of the earliest programs of this kind, \textsc{ZORK}, originally released in 1979 by members of the MIT Dynamic Modelling Group (ref: wp). In the game, the player types what he wants his virtual character to do in the game world, the game responds with a description of what effects these actions produce. The tutorials created with the framework described in this thesis work similary, in that they provide direct feedback to the user's commands and provide support when he makes mistakes or has problems (note ZORK's response to \texttt{examine mailbox}).
...
In the technical community, there have been several approaches in creating interactive tutorials for command line interfaces.

\emph{Try Ruby} was an interactive online tutorial that would provide an introduction to the Ruby scripting language. An anonymous programmer called \_why first published it at the end of 2005.\cite{tryrubydone} He took the site offline in early 2009, but some members of the Ruby community continue to develop it. (persistence?)

The American company \emph{Codecademy} currently deploys several interactive tutorials, which focus on teaching programming in Ruby, Python, JavaScript and PHP. The tutorials consist of multiple exercises, in each of which the user gets a piece of unfinished code and an exercise and then is supposed to fix or finish the code so that it fulfills some conditions.
...
While these implementations of interactive tutorials pose drastic improvements over classical tutorials, they still have several problems:

The presented tutorials are linear: Each task has to be solved to get to the next one.

Additionally, they do not offer that level of persistence that normal interaction with the command line would: As every entered line is run seperately, the system has no internal state.
...
This thesis covers design, implementation, application and evaluation of the \emph{Nut Shell}. We present the design of the framework in a bottom-up fashion:

In \cref{sec:preliminaries}, we introduce and define some of the framework's central topics and techniques.
...
\end{nutsh}
%
The \texttt{say} function supports two ways of highlighting parts of the text: Text enclosed in backticks will be displayed in a second color, text enclosed in asterisks in a third. As a convention, backticks are used to mark parts of commands or file names, and asterisks are used to emphazise parts of a sentence. These conventions have been adopted from John Gruber's Markdown \url{http://daringfireball.net/projects/markdown/syntax}.
