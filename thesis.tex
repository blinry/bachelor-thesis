...
Leslie Wöhler
\\[\baselineskip]
This document was created using \LaTeXe\ and \KOMAScript\ and edited with \textsc{Vim}. The figures are created with \texttt{pgfplots}, key combinations with \texttt{menukeys}. The \textsc{Latin Modern} font family.
}
\begin{document}
...
In this thesis, we treat the target command line process as a black box having input and output streams of Unicode characters. To recognize which commands the user entered and what the corresponding output is, the framework needs to parse the output accordingly.

For this purpose, we use two special \emph{markers}, which enable the parser to recognize the different parts. These are special character sequences that do not appear in normal output. (clarify)

\section{Recognizing the command}

While the user edits the command line, they can use many of readline's key bindings or other shell builtin key combinations. These make it hard to recognize which command he entered, as parts of the entered characters could have been deleted or otherwise changed. To solve this, we use a mechanism to repeat the entered line before it is sent.

All targets support (by definition, through Readline) the keyboard shortcuts \keys{\ctrl+U} and \keys{\ctrl+Y}, which delete the momentarily entered command and put it in an internal buffer / re-insert it. The parser waits for the user to press Enter and then sends the sequence
...
Here is a demonstration of this technique. The first column lists the user's keystrokes, the second one contains the characters sent to the process, the third one contains the output of the process, and the final one shows the current content of the terminal.
...
The \emph{Nut Shell} configures the prompt to start and end with a marker. This leads to problems when the user tries to reconfigure the prompt.
