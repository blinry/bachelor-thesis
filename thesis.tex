...
When the user presses \emph{return}, the described sequence is sent to the process, and when reading the “Output” column, the correct command \texttt{1+1} appears neatly between the two markers. After the first marker, there are some deletion characters in the output (they depend on the concrete command line program, but come from a fixed character set, see \cref{sec:esc}), and before the second marker there is always a space character. By removing these, the exact command the user wanted to enter is left.
