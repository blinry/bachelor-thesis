\documentclass[paper=a4,abstract=on,cleardoublepage=empty,numbers=noenddot,toc=bib]{scrreprt}
...
Leslie Wöhler
\\[\baselineskip]
This document was created using \LaTeXe\ and \KOMAScript\ and edited with \textsc{Vim}. The text is set using the \textsc{Latin Modern} font family.
}
\begin{document}
...
\begin{figure}[h]
    \caption{Average scores for the general questions}
    \label{tab:generalscores}
    \begin{tabular}{l|l|l}
        Question & average score Nut Shell & average score Paper \\
        \hline
        Ich hatte bereits Vorwissen über die Kommandozeile.        & 3.13 & 1.75 \\
        Der Vorkurs hat mir Spaß gemacht.                          & 8.50 & 7.75 \\
        Ich habe im Vorkurs viel gelernt.                              & 8.46 & 7.75 \\
        Die Aufgaben waren zu schwierig.                               & 3.98 & 4.33 \\
        Es war genug Zeit, die Aufgaben zu bearbeiten.                 & 8.31 & 8.83 \\
        Der Stoff erscheint mir relevant für meine weitere Ausbildung. & 7.90 & 7.58 \\
        Ich würde den Vorkurs weiterempfehlen.                     & 8.71 & 8.67
    \end{tabular}
\end{figure}
...
Figure~\ref{fig:participants} shows the number of participants per day in both groups.
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
        ...
        \begin{axis}[
            ...
            \addplot [pattern=north west lines, bar shift=-.2cm] table [y=Slot1, x expr=\coordindex] {\datatable};    % Plot the "First" column against the data index
            \addplot [pattern=north west lines, bar shift=-.2cm] table [y=Slot2, x expr=\coordindex] {\datatable};
            \addplot [pattern=north west lines, bar shift=-.2cm] table [y=Slot3, x expr=\coordindex] {\datatable};

            \resetstackedplots

            \addplot [pattern=north east lines, bar shift=.2cm] table [y=a, x expr=\coordindex] {\datatable};
            \addplot [pattern=north east lines, bar shift=.2cm] table [y=b, x expr=\coordindex] {\datatable};
            \addplot [pattern=north east lines, bar shift=.2cm] table [y=c, x expr=\coordindex] {\datatable};
