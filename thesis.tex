...
And finally, I want to thank the 120 students who bravely participated in the very first evaluation of the Nut Shell and gave helpful and encouraging feedback.
...
When reading the “Output” column, the correct command \texttt{1+1} now appears neatly between the two markers. The first marker is followed by some deletion sequences (they depend on the concrete command line program, but come from a fixed set\todo{specify!}. And the command is always followed by a space character. By removing both, the exact command the user wanted to enter can be extracted.
...
\todo{This section is still incomplete.}
