...
\titlehead{\includegraphics[height=2.5cm]{tu-logo.pdf} \hfill \includegraphics[height=2.5cm]{ips.pdf}}
...
Finally, I want to thank the 120 students who participated in the very first evaluation of the Nut Shell and gave helpful and encouraging feedback.
...
\Cref{sec:cliparser} describes the abstraction layer that communicates with the command line process at the core of each Nut Shell tutorial. It explains and demonstrates techniques for adapting to many different command line interfaces and names the requirements the CLIs need to fulfill.
...
When the user wants to execute a command, some \textsc{CLI}s check whether this command is complete. When it is not, they do not execute the command, but give him the possibility to complete it instead. In this case, the CLI usually displays a secondary prompt.
