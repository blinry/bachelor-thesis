...
Lesson files should be as easy to read and to write as possible. We decided to use a syntax close to the ones potential users will already know, that is, regular expressions and for the rest a syntax similar to Go. C users will also feel at home. The language contain syntactic structures that support often-used semantical statements. Redundance is minimized---the language provides mechanisms to reuse code snippets.

The language is string-based, the only data type is a string of Unicode characters. This was done to keep the language minimal. \emph{nutsh} does not have variables, which makes the language functional.
...
Identifiers serve as names that can be used for functions. An identifier is a non-empty sequence of alphanumeric characters.
...
\end{ebnf}
%
There are two builtin functions called \texttt{command} and \texttt{output}, that correspond to the user's last command and its output. When no prompt has occurred yet, they both return empty strings.
...
To define a new function, the \texttt{def} keyword is used, followed by the name of the function, optional arguments and a block. Note that the brackets around the arguments can be omitted if a function has no arguments:
...
A \emph{nutsh} file consists of a number of function definitions and other \texttt{Line}s (\texttt{if}-, \texttt{prompt}- and nesting statements as well as \emph{calls}, see \cref{sec:statements}). Function definitions can only appear at the top level to avoid redefinitions in different scopes, which would lead to name masking problems and more complexity.
...
As the author of a tutorial, one wants to verify that all the lessons work correctly. This is why the framework provides a method for automated testing of correctness, so one does not have to enter all the required commands by hand.

To help with this, the framework provides the builtin method \texttt{expect}, which expresses the assumption of the author that if a user were to enter the supplied argument as a command in the innermost surrounding \texttt{prompt} statement, the \texttt{expect} statement would be reached.
...
The material was divided into 30 invidiual lessons that covered one topic, respectively. In total, the tutorial contains 2875 lines of \textit{nutsh} code. In total, it took approximately 40 hours to write and test the lessons.
